...
export default function NuevoAumentoPage() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // Simular guardado en localStorage para demo
    const aumentosExistentes = JSON.parse(localStorage.getItem("aumentos") || "[]")
    aumentosExistentes.push(nuevoAumento)
    localStorage.setItem("aumentos", JSON.stringify(aumentosExistentes))

    alert(`Aumento ${formData.aplicarInmediatamente ? "aplicado" : "programado"} correctamente`)
  ...
  return (
    <div className="p-4 space-y-6">
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Nuevo Aumento de Precios</h1>
          <p className="text-gray-600">Configura un aumento de precios con criterios específicos</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="flex flex-col sm:flex-row gap-4 justify-end">
          ...
          >
            ...
            {formData.aplicarInmediatamente ? "Aplicar Aumento" : "Programar Aumento"}
          </Button>
        </div>
      </form>
    </div>
